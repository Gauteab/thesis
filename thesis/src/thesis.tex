\documentclass[a4paper,english]{ifimaster}

\usepackage[utf8]{inputenc}
\usepackage{babel,duomasterforside}
\usepackage{hyperref}
\usepackage[backend=biber,style=authoryear,bibencoding=utf8]{biblatex}
\usepackage{graphicx}
\graphicspath{ {./images/} }
\usepackage{booktabs}
\usepackage[utf8]{inputenc}
\usepackage[chapter]{minted}
\usepackage{subfiles}
\usepackage[toc,page]{appendix}
\usepackage{xcolor}
\usepackage{tikz}
% \usepackage{listings}
\usepackage{varioref}
\usepackage{multicol}

\usepackage{xcolor,pifont}
\usepackage{amsmath}
\usepackage[thmmarks, amsmath, framed]{ntheorem}%
\usepackage{cleveref}
\usepackage{enumitem}
\usepackage[font=small,labelfont=bf,tableposition=top]{caption}
\DeclareCaptionLabelFormat{andtable}{#1~#2  \&  \tablename~\thetable}
\setlist[enumerate]{label*=\arabic*.}
\theoremstyle{break}
\theoremheaderfont{\bfseries}
\theorembodyfont{\mdseries}
\theoremseparator{\smallskip }

\newtheorem{example}{Example}[section]

% Style Chapters
\usepackage[T1]{fontenc}
\usepackage{titlesec, color}
\definecolor{gray75}{gray}{0.75}
\newcommand{\hsp}{\hspace{20pt}}
\titleformat{\chapter}[hang]{\Huge\bfseries}{\thechapter\hsp\textcolor{gray75}{|}\hsp}{0pt}{\Huge\bfseries}

\nonstopmode{}
\addbibresource{citations.bib}

% \newcommand*{\xref}[1]{\hyperref[{#1}]{\Vref*{#1}, \nameref*{#1}}} % One single link
% \newcommand{\xref}[1]{\Ref{#1} \nameref{#1} \vpageref[\unskip]{#1}}

\newenvironment{code}[4][]
{\VerbatimEnvironment
    \begin{listing}[!htb]
        \caption{#3}\label{#4}
    \begin{minted}[
        frame=lines,
        #1]{#2}}
 {\end{minted}\end{listing}}

\AtBeginEnvironment{minted}{%
    \renewcommand{\fcolorbox}[4][]{#4}}

\renewcommand\listoflistingscaption{Listings}

\title{Leveraging Language Tooling for Better Voice Coding}
\subtitle{Implementing program awareness and structural editing for Talon}
\author{Gaute Berge}

\begin{document}

\duoforside[dept={Department of Informatics}, program={Informatics: Programming and System Architecture}, long]
\frontmatter{}
\subfile{chapters/abstract}
\subfile{chapters/acknowledgments}
\tableofcontents{}
\listoffigures{}
\listoftables{}
\listoflistings{}
\subfile{chapters/preface}
\mainmatter{}

\subfile{chapters/introduction}
\subfile{chapters/background}
\subfile{chapters/talon}
\subfile{chapters/methodology}
\subfile{chapters/project}
\subfile{chapters/results}
\subfile{chapters/discussion}
\subfile{chapters/conclusions}

% Donald Knuth often says smart stuff ~\parencite{Knuth:2007:CPA:1283920.1283929}.
\backmatter{}
\printbibliography{}
\subfile{chapters/appendix}
\end{document}
